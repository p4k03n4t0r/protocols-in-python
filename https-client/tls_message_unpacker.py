...
class TLS_Message_Unpacker:
    ...
    @staticmethod
    def unpack_tls_message(
        message_type_bytes, message_version_bytes, record_length_bytes, raw_content
    ):
        ...
        # Application Data (x17/23)
            ...
            tls_message.additional_data = (
                message_type_bytes + message_version_bytes + record_length_bytes
            )
        else:
            raise Exception(
                "Can't handle this message type yet: {}".format(message_type_bytes)
            )
        return tls_message
    ...
    def parse_handshake_content(tls_message, raw_handshake):
        ...
        handshake_content_length = int.from_bytes(
            raw_handshake[i : i + 3], TLS_Message_Unpacker.ENDINESS
        )
        i += 3
        ...
        # CertificateVerify (x0f\15)
            ...
            TLS_Message_Unpacker.parse_certificate_verify(
                tls_message, handshake_content
            )
        # Finished (x14\20)
            ...
            raise Exception(
                "Received handshake type that is unknown or can't be parsed yet: {}".format(
    ...
    def parse_hello(tls_message, handshake_content):
        ...
        while i < len(handshake_content):
            ...
            extension_length = int.from_bytes(
                handshake_content[i : i + 2], tls_message.ENDINESS
            )
            i += 2
            ...
            # Extension: Key Share
                ...
                if extension_length > 2:
                    key_exchange_length = int.from_bytes(
                        handshake_content[i : i + 2], tls_message.ENDINESS
                    )
                    i += 2
                    tls_message.key_exchange = handshake_content[
                        i : i + key_exchange_length
                    ]
    ...
    def parse_application_data(application_data):
        ...
        tls_message = TLS_Message_Unpacker.unpack_tls_message(
            b"\x16", b"\x03\04", len(application_data), application_data
        )
        return tls_message
    ...
    def parse_certificate_data(tls_message, handshake_content):
        ...
        certificates_length = int.from_bytes(
            handshake_content[i : i + 3], TLS_Message_Unpacker.ENDINESS
        )
        i += 3
    ...
    def parse_certificate_verify(tls_message, handshake_content):
        ...
        signature_length = int.from_bytes(
            handshake_content[i : i + 2], TLS_Message_Unpacker.ENDINESS
        )
        i += 2
