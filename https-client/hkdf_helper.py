...
class Hkdf_Helper:
    ...
    def hkdf_expand_label(
        pseudo_random_key, label, context, length=32, hash_algorithm=hashlib.sha256
    ):
        # create info:
        # struct {
        ...
        return Hkdf_Helper.hkdf_expand(
            pseudo_random_key, hkdf_label, length, hash_algorithm
        )

    @staticmethod
    def hkdf_extract_expand_label(
        salt,
        input_key_material,
        label,
        context,
        ...
        extract_result = Hkdf_Helper.hdkf_extract(
            salt, input_key_material, hash_algorithm
        )
        return Hkdf_Helper.hkdf_expand_label(
            extract_result, label, context, length, hash_algorithm
        )
