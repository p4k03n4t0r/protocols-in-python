...
class TLS_Connection:
    ...
    def send(self, tls_message):
        ...
        message_bytes, transcript_message_bytes = TLS_Message_Packer.pack_tls_message(
            tls_message,
    ...
    def receive(self):
        ...
        tls_message = TLS_Message_Unpacker.unpack_tls_message(
            message_type_bytes,
        ...
        if tls_message.message_type == b"\x15":
            ...
            print(
                "Level: {}, Description: {}".format(level_message, description_message)
            )
        ...
        if tls_message.message_type == b"\x17":
            message_bytes = self.decrypt_response(
                tls_message.application_data, tls_message.additional_data
            )
    ...
    def update_transcript(self, tls_message, message_bytes, remove_record_header):
        ...
        if tls_message.message_type in [b"\x16", b"\x17"]:
            ...
            if tls_message.handshake_type in [
                b"\x01",
                b"\x02",
                b"\x08",
                b"\x0d",
    ...
    def calculate_keys(self):
        ...
        self.server_public_key = Crypto_Helper.get_public_key_from_shared_key(
            self.server_shared_key, self.cryptographic_group
        )
        if self.client_private_key is None:
            raise Exception("client_private_key must be set to calculate the keys!")
        self.shared_secret = Crypto_Helper.get_shared_secret(
            self.client_private_key, self.server_public_key, self.cryptographic_group
        )
        ...
        ) = Crypto_Helper.derive_keys(cipher_suite, self.shared_secret, transcript_hash)
    ...
    def decrypt_response(self, message, additional_data):
        ...
        decrypted_message = Crypto_Helper.aead_decrypt(
            message, additional_data, server_handshake_key, server_handshake_iv, counter
        )
    ...
    def verify_certificate(self, server_signature_algorithm, server_signature):
        ...
        transcript_hash = Crypto_Helper.hash_transcript(
            self.cipher_suite, self.transcript_bytes
        )
        dignital_signature += transcript_hash

        # verify signature
        Crypto_Helper.verify_certificate_signature(
            server_signature,
            dignital_signature,
    ...
    def verify_data(self, server_verify_data):
        ...
        Crypto_Helper.verify_data(
            server_verify_data, self.server_handshake_traffic_secret, transcript_hash
        )
