...
class Crypto_Helper:
    ...
    def generate_client_keys(cryptographic_group):
        if cryptographic_group == "x25519" or cryptographic_group == "x448":
            ...
            or cryptographic_group == "secp384r1"
            or cryptographic_group == "secp521r1"
        ):
    ...
    def get_public_key_from_shared_key(shared_key, cryptographic_group):
        ...
        yBin = shared_key[
            coordinate_byte_size : coordinate_byte_size + coordinate_byte_size
        ]
    ...
    def get_shared_secret_secpr1(client_private_key, server_public_key, curve_class):
        client_private_key_int = int.from_bytes(
            client_private_key, Crypto_Helper.ENDINESS
        )
        private_key = derive_private_key(
            client_private_key_int, curve_class(), default_backend()
        )
        server_public_key_int = int.from_bytes(
            client_private_key, Crypto_Helper.ENDINESS
        )
        public_key = derive_private_key(
            server_public_key_int, curve_class(), default_backend()
        )
    ...
    def derive_keys(cipher_suite, shared_secret, transcript_hash):
        ...
        if cipher_suite != b"\x13\x01":
            raise Exception(
                "Only cipher suite TLS_AES_128_GCM_SHA256 is supported for now"
            )

        empty_hash = hashes.Hash(hashes.SHA256(), default_backend()).finalize()
        derived_secret = Hkdf_Helper.hkdf_extract_expand_label(
            b"\x00" * 32, b"\x00" * 32, b"derived", empty_hash
        )

        handshake_secret = Hkdf_Helper.hdkf_extract(derived_secret, shared_secret)
        client_handshake_traffic_secret = Hkdf_Helper.hkdf_expand_label(
            handshake_secret, b"c hs traffic", transcript_hash
        )
        server_handshake_traffic_secret = Hkdf_Helper.hkdf_expand_label(
            handshake_secret, b"s hs traffic", transcript_hash
        )

        client_handshake_key = Hkdf_Helper.hkdf_expand_label(
            client_handshake_traffic_secret, b"key", b"", 16
        )
        client_handshake_iv = Hkdf_Helper.hkdf_expand_label(
            client_handshake_traffic_secret, b"iv", b"", 12
        )
        server_handshake_key = Hkdf_Helper.hkdf_expand_label(
            server_handshake_traffic_secret, b"key", b"", 16
        )
        server_handshake_iv = Hkdf_Helper.hkdf_expand_label(
    ...
    @staticmethod
    def aead_decrypt(
        ciphertext, additional_data, server_handshake_key, server_handshake_iv, counter
    ):
    ...
    @staticmethod
    def aead_encrypt(
        text, additional_data, client_handshake_key, client_handshake_iv, counter
    ):
        # from https://cryptography.io/en/latest/hazmat/primitives/symmetric-encryption/#cryptography.hazmat.primitives.ciphers.modes.GCM
        # the tag is 16 bytes is long and appended at the end of the ciphertext, it's used to check the cipher text isn't tampered with
        # more info https://bensmyth.com/files/Smyth19-TLS-tutorial.pdf (page 31)
    ...
    def verify_certificate_signature(
        ...
        if signature_algorithm in [b"\x04\x01", b"\x04\x03", b"\x08\x04", b"\x08\x09"]:
            ...
            raise Exception(
                "Signature hash not supported: {}".format(signature_algorithm)
            )
        ...
        if signature_algorithm in [b"\x04\x01", b"\x05\x01", b"\x06\x01"]:
            ...
            signature_padding = padding.PSS(
                mgf=padding.MGF1(signature_hashes), salt_length=padding.PSS.MAX_LENGTH
            )
        else:
            raise Exception(
    ...
    def verify_data(expected_data, server_handshake_traffic_secret, finished_hash):
        finished_key = Hkdf_Helper.hkdf_expand_label(
            server_handshake_traffic_secret, b"finished", b""
        )
        actual_data_hmac = hmac.HMAC(
            finished_key, hashes.SHA256(), backend=default_backend()
        )
