...
if "TOTAL_MATCH_CHUNKED" in os.environ:
    if os.environ["TOTAL_MATCH_CHUNKED"] == "True":
        ...
        raise Exception(
            "Value for TOTAL_MATCH_CHUNKED must be either 'True' or 'False'"
        )
            ...
            while connection_has_data(received_connection):
                ...
                try:
                    http_message = Http_Message(
                        received_connection, total_match_chunked
                    )
                    print("Message received:")
                    print(
                        "{} {} {}".format(
                            http_message.http_method,
                    ...
                    if is_proxy:
                        ...
                        if http_message.request_target == "/flag":
                            ...
                            with socket.socket(
                                ...
                                try:
                                    while True:
                                        http_response += proxy_socket.recv(1).decode(
                                            "ascii"
                                        )
